...
FOLLOWS_PER_DAY = 100
FOLLOW_PERIOD = datetime.timedelta(seconds=10)  # TODO minutes=1)
...
def update_followers(db, user, twitter_id):
    with db, db.cursor() as cursor:
        ...
        updated_time = database.get_twitter_followers_updated_time(cursor, twitter_id)
    logging.info('followers for %s last updated at %s', twitter, updated_time)
    if updated_time and now() - updated_time < UPDATE_PERIOD: return
    ...
    if updated_time:  # delete old followers, since they no longer are
        with db, db.cursor() as cursor:
...
def follow(db, user, twitter_id):
    ...
    with db, db.cursor() as cursor:
        followed_time = database.get_user_followed_time(cursor, user.id)
    ...
    if followed_time and now() - followed_time < FOLLOW_PERIOD: raise Exception('user followed too recently')
