...
def get_user_mentor_ids(cursor, user_id, updated_before=None):
    sql = 'select mentor_id from user_mentors where user_id=%s'
    values = [user_id]
    if updated_before:
        sql += ' and (followers_cursor is not null or now() - updated_time > %s)'
        values.append(updated_before)

    cursor.execute(sql, values)
    return [row.id for row in cursor.fetchall()]

def get_user_unfollowed_mentor_follower_ids(cursor, user_id, mentor_id):
    cursor.execute('select follower_id from twitter_followers left join user_follows on follower_id=followed_id')
